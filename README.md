...
Connect to `ws://<host>/ws/comments/` to receive real-time events. The channel is **server-push only** — clients connect and listen, sending is ignored.
...
Five tables; all primary keys are UUIDs.
...
captcha_captchastore    (django-simple-captcha managed)
  hashkey       VARCHAR  PK
  challenge     VARCHAR
  response      VARCHAR
...
This section maps the implementation to the assessment checklist and documents decisions that go beyond the stated requirements.

### Junior+ Level Checklist

| Requirement | Implementation |
|-------------|----------------|
| ✅ **Async task queue** | Celery 5 + Redis broker. `notify_reply_author` retries up to 3× with 60 s backoff on failure (`bind=True`, `self.retry(exc=exc)`) |
| ✅ **Redis cache** | `django-redis` on DB 1. List pages cached 60 s (short — writes are frequent). Trees cached 5 min (rarely updated). Pattern-based bulk invalidation with `redis.keys("comments:list:*")` |
| ✅ **Real-time events** | Django Channels 4 `AsyncWebsocketConsumer`. Signal fires via `transaction.on_commit()` — guarantees no broadcast for a rolled-back row |
| ✅ **JWT identity token** | `djangorestframework-simplejwt` `Token` subclass. Issued on comment creation, 30-day lifetime. Encodes `user_id`, `username`, `email`. Not an auth token — grants no permissions |
